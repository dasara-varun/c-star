...
const FEATURE_VERSIONS: &[&str] = &[
    "llvm11-0", "llvm12-0", "llvm13-0", "llvm14-0", "llvm15-0", "llvm16-0", "llvm17-0", "llvm18-1", "llvm19-1",
...
pub fn expand(args: Option<VersionRange>, input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let res: proc_macro::TokenStream = match VersionFolder::fold_any(Fold::fold_item, item) {
        Ok(item) => quote! { #args #item },
        Err(err) => {
            let err = err.into_compile_error();
            let input = proc_macro2::TokenStream::from(input_clone.clone());
            quote! { #err #args #input }
        },
    }
    .into();
    eprintln!("INKWELL_MACRO: input = {:?}, output = {:?}", input_clone.to_string(), res.to_string());
    res
